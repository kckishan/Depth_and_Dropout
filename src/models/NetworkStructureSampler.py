...
class NetworkStructureSampler(nn.Module):
    ...
    def __init__(self, args, device):
        ...
        # Truncation level for approximation
        ...
        self.prior_beta_dist = Beta(self.α, self.β)
    ...
    def get_kl(self):
        ...
        posterior_beta_dist = Beta(a_k, b_k)
        kl_beta = kl_divergence(posterior_beta_dist, self.prior_beta_dist)
        return kl_beta.sum()
    ...
    def forward(self, num_samples=5, get_pi=False):
        ...
        ν = posterior_beta_dist.rsample((num_samples, )).view(num_samples, self.truncation_level) # S x K
        ...
        # sample binary mask z given the activation level π of the layer
        if self.training:
            # draw continuous binary sample from concrete Bernoulli distribution to backpropagate the gradient
            concrete_bernoulli_dist = RelaxedBernoulli(probs=π, temperature=self.temperature)
        ...
        # If there are no activated neurons, add a hidden layer
        if threshold == 0:
